...
known_ecosites <- ecosites %>%
  ...
  drop_na() %>% 
  left_join(summary, ., by = c('Plot.ID' = 'PLOT.ID')) %>% 
...
The Ecological Site Groups do not contain values for bare ground cover, however they do contain the mean value of 'Total Foliar Cover' for each AIM plot within the concept. In order to generate a realistic estimate of Bare ground for these plots, we simply assume that the cover of Litter, Rock Fragments and Biocrusts are in equilibrium at AIM plots, and add these onto the Foliar estimate

$$ 100 - (\text{ESG total foliar}  + (\text{AIM litter} +  \text{AIM rock} + \text{AIM biocrusts})) = \text{Bareground} $$


```{r Create ESG estimates}
