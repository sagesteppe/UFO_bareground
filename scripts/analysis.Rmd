...
The Ecological Site Groups do not contain values for bare ground cover, however they do contain the mean value of 'Total Foliar Cover' for each AIM plot within the concept. In order to generate a realistic estimate of Bare ground for these plots, we simply assume that the cover of Litter, Rock Fragments and Biocrusts are in equilibrium at AIM plots. In making this assumption, we may simple compare the estimates of total foliar cover, from the TerrAdat dataset to the estimates of total cover from the Ecological Site Groups. 

$$ 100 - (\text{ESG total foliar}  + (\text{AIM litter} +  \text{AIM rock} + \text{AIM biocrusts})) = \text{Bare ground} $$
...
newdat <- newdat %>%
  ...
  mutate(BareSoilCover = if_else(BareSoilCover < 5, 5, BareSoilCover)) %>% 
  rename(Imputed_Target = BareSoilCover) %>% 
  mutate(Imputed_Target = mround(Imputed_Target, base= 5))

t <- esg_bm2 %>% 
  mutate(TotalFoliarCover = round(TotalFoliarCover, 0)) %>% 
  left_join(., newdat, by = 'TotalFoliarCover') %>% 
  mutate(Meeting2 = if_else(BareSoilCover <= Imputed_Target, T, F),
         .after = BareSoilCover) 
